...
function UseCaseCard({ useCase, isActive }) {
  ...
  return (
    <motion.div
      layout
      className={`bg-white rounded-2xl border ${isActive ? 'border-violet-300 shadow-lg' : 'border-gray-200'} overflow-hidden h-full flex flex-col`}
    >
      {/* Header */}
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            The First
            <span className="bg-gradient-to-r from-violet-600 to-cyan-500 bg-clip-text text-transparent"> "AI Discovery Engine"</span>
            <br />Platform.
          </h2>
              ...
              <div className="flex gap-2">
                {useCases.map((_, idx) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-colors ${idx === activeIndex ? 'bg-violet-600' : 'bg-gray-300'}`}
                  />
                ))}
              </div>
              <button 
